...
class LaptopController extends Controller
{
    ...
    function load_data(Request $request)
    {
        if ($request->ajax())
        {
            ...
            {
                $data = DB::table('td_product')->where('prod_featured', 1)
                    ->where('prod_cate', 2)
                    ->orderBy('prod_id', 'DESC')
                    ->limit(5)
                    ->get();
    ...
    {
        $data['laptopsale'] = Product::where('prod_featured', 4)->Where('prod_cate', 2)
            ->orderBy('prod_id', 'desc')
            ->take(10)
            ->get();
        // laptop gia soc
        ...
        $data['laptop2'] = Product::where('prod_featured', 3)->Where('prod_cate', 2)
            ->orderBy('prod_id', 'desc')
            ->take(1)
            ->get();

        $data['laptop4'] = Product::where('prod_featured', 0)->Where('prod_cate', 2)
            ->orderBy('prod_id', 'desc')
            ->take(3)
            ->get();

        $data['laptop1'] = Product::where('prod_featured', 1)->Where('prod_cate', 2)
            ->orderBy('prod_id', 'desc')
            ->take(3)
            ->get();
        $data['laptop3'] = Product::where('prod_featured', 1)->where('prod_cate', 2)
            ->orderBy('prod_id', 'desc')
            ->take(0)
            ->get();
    ...
    {
        $data['laptoplist'] = DB::table('td_product')->where('prod_cate', 2)
...
{
  $data['product']=Product::where('prod_price','<',10000000)->where('prod_cate',2)->orderBy('prod_id','desc')->paginate(4);
